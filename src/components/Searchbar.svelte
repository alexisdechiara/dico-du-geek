...
<div class="relative {size === 'lg' ? 'w-11/12 sm:w-3/4 md:w-2/3 lg:w-1/2' : 'w-1/3'}">
	...
	<form
		...
		class="flex justify-between items-center bg-light-base border border-light-border dark:border-dark-border text-light-primary dark:text-dark-primary {formClass}"
	>
		<input
			type="text"
			id="searchInput"
			class="w-full placeholder:font-normal border-0 ring-0 outline-0 {inputClass}"
		...
		<button
			...
			class="flex aspect-[3/2] justify-center items-center text-light-background bg-light-ternary hover:bg-light-secondary dark:bg-dark-ternary dark:hover:bg-dark-secondary transition-colors duration-200 {buttonClass}"
			on:click={handleClickLink}
		>
			<svg xmlns="http://www.w3.org/2000/svg" fill="inherit" id="Outline" viewBox="0 0 24 24" class="fill-dark-primary {iconClass}"
	...
	{#if words.length > 0 && words != null && isVisible}
		<ul class="absolute bg-light-base flex flex-col w-full p-2 shadow-sm border border-light-border {listClass}">
			{#each words as word}
				<li>
					<a href={`/${word.slug}/definition`} class="flex hover:bg-light-background hover:text-light-secondary rounded {linkClass}"
